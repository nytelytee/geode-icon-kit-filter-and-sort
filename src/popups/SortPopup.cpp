...
void SortPopup::recalculateListItemBackgrounds() {
  for (int i = 0; i < SORT_TYPE_COUNT; i++) {
    CCLayerColor *listItemForeground = m_sortingList->getChildByIndex<CCLayerColor>(5*i+4);
...
void SortPopup::onArrow(CCObject *sender) {
  ...
  CCMenu* menu = m_sortingList->getChildByIndex<CCMenu>(menuIndex);
  CCMenu* swapWithMenu = m_sortingList->getChildByIndex<CCMenu>(swapWithMenuIndex);

  CCMenuItemSpriteExtra *sortOrder = menu->getChildByIndex<CCMenuItemSpriteExtra>(2);
  CCMenuItemSpriteExtra *swapWithSortOrder = swapWithMenu->getChildByIndex<CCMenuItemSpriteExtra>(2);

  std::string sortOrderState = sortOrder->getChildByIndex<SortIconButtonSprite>(0)->getBGName();
  std::string swapWithOrderState = swapWithSortOrder->getChildByIndex<SortIconButtonSprite>(0)->getBGName();
  sortOrder->getChildByIndex<SortIconButtonSprite>(0)->setBG(swapWithOrderState.c_str(), false);
  swapWithSortOrder->getChildByIndex<SortIconButtonSprite>(0)->setBG(sortOrderState.c_str(), false);

  CCLabelBMFont *label = m_sortingList->getChildByIndex<CCLabelBMFont>(labelIndex);
  CCLabelBMFont *swapWithLabel = m_sortingList->getChildByIndex<CCLabelBMFont>(swapWithLabelIndex);
...
void SortPopup::onReverseSortOrder(CCObject *sender) {
  ...
  if (iconKitState.pendingSettings.sortIsReverse[listItem])
    item->getChildByIndex<SortIconButtonSprite>(0)->setBG("GJ_button_01.png", false);
  else
