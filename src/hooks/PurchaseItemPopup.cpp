...
void HookedPurchaseItemPopup::onModify(auto& self) {
  ...
  if (!result && *result.err() != "Mod not found") log::error("Failed to set hook priority, Icon Kit Filter & Sort may crash with Better Unlock Info.");
}

void HookedPurchaseItemPopup::onPurchase(CCObject* sender) {
